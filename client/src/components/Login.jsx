...
export default function Login(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://keeper-app-kgxr.onrender.com/getdata")
      .then((response) => {
        change_details((prev_value) => {
          return {
            ...prev_value,
  ...
  function validate_user(event) {
    ...
    if (validate()) {
      ...
      if (!state) {
        ...
        axios({
          url: "https://keeper-app-kgxr.onrender.com/add",
          method: "POST",
          data: user_details,
        })
  ...
  return (
    <div className="login">
      <h1 style={{ marginBottom: "7vh" }}>{state ? "Login" : "Register"}</h1>
      <form onSubmit={validate_user}>
        ...
        {state ? null : (
          <div>
            ...
            />
            <div style={{ color: "#fff", fontSize: 15, marginBottom: 7 }}>
              {user_details.confirm_password_error}
            </div>
          </div>
        )}
      ...
      {state ? (
        <div>
          <a href="#" onClick={change_to_register}>
            {" "}
            Register{" "}
          </a>
        </div>
